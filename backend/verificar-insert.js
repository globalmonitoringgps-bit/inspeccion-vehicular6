...
async function verificarInsert() {
    const sql = `
    ...
    console.log(`✅ Placeholders en SQL: ${placeholders}`);
    console.log(`✅ Deberían ser: 106 (107 columnas - 1 columna id)`);
    console.log(`✅ ¿Coincide? ${placeholders === 106 ? '✅ SÍ' : '❌ NO'}`);
    
    // Contar columnas en el INSERT
    const columnasInsert = sql.match(/INSERT INTO Inspecciones\s*\(([^)]+)\)/s);
    if (columnasInsert) {
        const columnas = columnasInsert[1].split(',').map(c => c.trim()).filter(c => c);
        console.log(`✅ Columnas en INSERT: ${columnas.length}`);
    }
}
